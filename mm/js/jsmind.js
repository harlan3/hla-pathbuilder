...
; (function ($w) {
    ...
    jm.node = function (sId, iIndex, sTopic, sLink, sSemantics, oData, bIsRoot, oParent, eDirection, bExpanded) {
    ...
    jm.mind.prototype = {
        ...
        set_root: function (nodeid, topic, link, semantics, data) {
            if (this.root == null) {
                this.root = new jm.node(nodeid, 0, topic, link, semantics, data, true);
        ...
        add_node: function (parent_node, nodeid, topic, link, semantics, data, direction, expanded, idx) {
            if (!jm.util.is_node(parent_node)) {
                logger.error('the parent_node ' + parent_node + ' is not a node.');
                return null;
            }
            var node_index = idx || -1;
            var node = new jm.node(nodeid, node_index, topic, link, semantics, data, false, parent_node, parent_node.direction, expanded);
        ...
        insert_node_before: function (node_before, nodeid, topic, data, direction) {
            ...
            return this.add_node(node_before.parent, nodeid, topic, link, semantics, data, direction, true, node_index);
        },
        ...
        insert_node_after: function (node_after, nodeid, topic, data, direction) {
            ...
            return this.add_node(node_after.parent, nodeid, topic, link, semantics, data, direction, true, node_index);
        },
    ...
    jm.format = {
        node_tree: {
            ...
            _parse: function (mind, node_root) {
                ...
                mind.set_root(node_root.id, node_root.topic, node_root.link, node_root.semantics, data);
            ...
            _extract_data: function (node_json) {
                ...
                for (var k in node_json) {
                    if (k == 'id' || k == 'topic' || k == 'link' || k == 'semantics' || k == 'children' || k == 'direction' || k == 'expanded') {
                        continue;
                    }
                    data[k] = node_json[k];
                }
            ...
            _extract_subnode: function (mind, node_parent, node_json) {
                ...
                var node = mind.add_node(node_parent, node_json.id, node_json.topic, node_json.link, node_json.semantics, data, d, node_json.expanded);
            ...
            _extract_root: function (mind, node_array) {
                ...
                while (i--) {
                    if ('isroot' in node_array[i] && node_array[i].isroot) {
                        ...
                        var node = mind.set_root(root_json.id, root_json.topic, root_json.link, root_json.semantics, data);
            ...
            _extract_subnode: function (mind, parent_node, node_array) {
                ...
                while (i--) {
                    ...
                    if (node_json.parentid == parent_node.id) {
                        ...
                        var node = mind.add_node(parent_node, node_json.id, node_json.topic, node_json.link, node_json.semantics, data, d, node_json.expanded);
                        node_array.splice(i, 1);
                        extract_count++;
        ...
        freemind: {
            ...
            _load_node: function (mind, parent_node, xml_node) {
                ...
                var node = null;
                if (!!parent_node) {
                    node = mind.add_node(parent_node, node_id, node_topic, node_link, node_semantics, node_data, node_direction, node_expanded);
                } else {
                    node = mind.set_root(node_id, node_topic, node_link, node_semantics, node_data);
                }
    ...
    jm.prototype = {
        ...
        add_node: function (parent_node, nodeid, topic, link, semantics, data, direction) {
            if (this.get_editable()) {
                ...
                var node = this.mind.add_node(the_parent_node, nodeid, topic, link, semantics, data, dir);
                if (!!node) {
                    ...
                    this.invoke_event_handle(jm.event_type.edit, { evt: 'add_node', data: [the_parent_node.id, nodeid, topic, link, semantics, data, dir], node: nodeid });
                }
                return node;
            } else {
                logger.error('fail, this mind map is not editable');
    ...
    jm.view_provider.prototype = {
        ...
        create_node_element: function (node, parent_node) {
            ...
            d.setAttribute('onmouseover', 'display_semantics("' + node.semantics + '");');
            d.setAttribute('onmouseout', 'clear_semantics();');
